...
## Solve ...
   ...
   Result = hspcore(yd=SimDat, ORDER=4, Exterior.knots = c(0,10), Interior.knots=Interior.knots, SelectBestKnots = FALSE, time=timeout, Bootstrap = 150, verbose=FALSE )
...
plot( INPUTS$t, INPUTS$Sh, type="l", col="green", ylim=c(0,1.0),xlab="t",ylab="CV Survival Probability",bty="n")
...
## Solve ...
   ...
   Interior.knots
   # Then, we apply bootstrap to calculate the variance
   Result = hspcore(yd=SimDat, ORDER=3, Exterior.knots = c(0,10), Interior.knots=Interior.knots, SelectBestKnots = FALSE, time=timeout, Bootstrap = 150, verbose=FALSE )
     # Convergence issues with some algorithms
   print( paste0( "Times converged: L-BFGS-B ", sum( Result$convergence[,1]==0 ), ", PORT ", sum( Result$convergence[,2]==0 ),
                  ", Barrier ", sum( Result$convergence[,3]==0 ), ", BOBYQA ", sum( Result$convergence[,4]>0 ) ) )
    # which method found the minimum objective?
