...
#'
#' @details Times to event are defined as t = min( t_h, t_C), where t_h is the time to hazard (distributed according to the input S_h) and t_C is the time to censoring (distributed according to the input S_censoring)
#'
...
etsim = function( INPUTS ){
  ...
  sel = which( unif >= min(unif_x) & unif <= max(unif_x) )
  timeCensor[sel]   = pracma::interp1( unif_x, unif_y, unif[sel], method="linear" )
